...
extern "C" __global__ __aicore__ void abs_pad_custom(GM_ADDR inputGM, GM_ADDR outputGM, GM_ADDR workspace,
                                                         GM_ADDR tilingData);
#endif
extern uint8_t *GenerateTiling(const std::vector<int64_t> shapePad, const std::vector<int64_t> shapeUsed);
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    memcpy_s(tiling, tilingSize, GenerateTiling(shapePad, shapeUsed), tilingSize);
    ReadFile("./input/input_x.bin", inputByteSize, inputGM, inputByteSize);

    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    ICPU_RUN_KF(abs_pad_custom, blockDim, inputGM, outputGM, workspace, tiling); // use this macro for cpu debug
    ...
    uint8_t *xHost, *zHost, *workspaceHost;
    ...
    CHECK_ACL(aclrtMemcpy(tilingDevice, tilingSize, GenerateTiling(shapePad, shapeUsed), tilingSize,
