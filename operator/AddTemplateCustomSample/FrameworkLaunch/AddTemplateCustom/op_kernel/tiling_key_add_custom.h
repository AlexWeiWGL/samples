...
ASCENDC_TPL_ARGS_DECL(args0, ...):算子的模板参数定义, args0表示算子唯一标识, 建议与opType保持一致,后续为若干个DTYPE、FORMAT、UINT、BOOL的模板参数定义
    ASCENDC_TPL_DTYPE_DECL(args0, ...): DTYPE类型的模板参数定义,args0表示参数名,后续若干个参数为穷举的DTYPE枚举值,DTYPE枚举值参见metadef对外定义的数据类型
    ASCENDC_TPL_FORMAT_DECL(args0, ...): FORMAT类型的模板参数定义,args0表示参数名,后续若干个参数为穷举的FORMAT枚举值,FORMAT枚举值参见metadef对外定义的数据类型
    ASCENDC_TPL_UINT_DECL(args0, args1, args2, ...): UINT类型的模板参数定义
        ...
        args2: 后续...参数表示类型,支持的表示类型为 ASCENDC_TPL_UI_RANGE,ASCENDC_TPL_UI_LIST,ASCENDC_TPL_UI_MIX
          ...
          实例说明2 ASCENDC_TPL_UI_LIST,10,12,13,9,8,7,6：则表示1组穷举参数,[10, 12, 13, 9, 8, 7, 6]为穷举值,因此该参数定义的UINT参数合法值为{10, 12, 13, 9, 8, 7, 6},并且后续编码将以这个定义的顺序编码
          实例说明1 ASCENDC_TPL_UI_MIX,2,0,2,3, 5, 7, 6：则表示2组穷举参数,这2组范围为{0, 2}, {3, 5};[7, 6]为穷举值,因此该参数定义的UINT参数合法值为{0, 1, 2, 3, 4, 5, 7, 6},并且后续编码将以这个定义的顺序编码
        【使用注意事项】:
...
*/
ASCENDC_TPL_ARGS_DECL(AddTemplateCustom, //唯一标识
ASCENDC_TPL_DTYPE_DECL(X, ADD_TPL_FP16, ADD_TPL_BP16, ADD_TPL_FP32), //编码位宽位8比特
ASCENDC_TPL_FORMAT_DECL(Y, ADD_TPL_ND, ADD_TPL_NZ), //编码位宽位4比特
ASCENDC_TPL_UINT_DECL(Z, ASCENDC_TPL_8_BW, ASCENDC_TPL_UI_MIX, 2, 0, 2, 3, 5, 7, 6),//编码位宽位为用户定义的ASCENDC_TPL_8_BW比特,注意用户指定的位宽需要大于定义的参数个数
ASCENDC_TPL_BOOL_DECL(S, 0, 1), //编码位宽位1比特
);
/**
...
ASCENDC_TPL_SEL(
    ...
    ASCENDC_TPL_DTYPE_SEL(X, ADD_TPL_FP16, ADD_TPL_BP16),
    ASCENDC_TPL_FORMAT_SEL(Y, ADD_TPL_ND),
    ASCENDC_TPL_UINT_SEL(Z, ASCENDC_TPL_UI_LIST, 4, 7),
    ASCENDC_TPL_BOOL_SEL(S, 0, 1),
    ),
    ASCENDC_TPL_ARGS_SEL(
    ASCENDC_TPL_DTYPE_SEL(X, ADD_TPL_FP32),
    ASCENDC_TPL_FORMAT_SEL(Y, ADD_TPL_ND,ADD_TPL_NZ),
